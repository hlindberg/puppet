...
#
    ...
    context 'When parsing given issue' do
      ...
      it 'Raises an exception if there is no reference to a benchmark in the given issue' do
        allow(@apply).to receive(:load_config) { {} }
        @apply.setup
    ...
    describe 'Issue' do
      it 'can be created by giving each part of the issue' do
        x = Puppet::Application::Fix::Model::Issue.new(mnemonic: 'abc', section: '1.1.1', name: 'test-issue')
      ...
      it 'can be created from a hash' do
        x = Puppet::Application::Fix::Model::Issue.new_from_hash({'mnemonic' => 'abc', 'section' => '1.1.1', 'name' => 'test-issue'})
        expect(x.name).to eq('test-issue')
        expect(x.mnemonic).to eq('abc')
        expect(x.section).to eq('1.1.1')
      end
      ...
      it 'can be created from a string' do
        x = Puppet::Application::Fix::Model::Issue.parse_issue('abc::1.1.1_test-issue')
        expect(x.name).to eq('test-issue')
        expect(x.mnemonic).to eq('abc')
        expect(x.section).to eq('1.1.1')

        x = Puppet::Application::Fix::Model::Issue.parse_issue('abc::1.1.1-test-issue')
        expect(x.name).to eq('test-issue')
        expect(x.mnemonic).to eq('abc')
        expect(x.section).to eq('1.1.1')
      end
      ...
      it 'eql? method compares attribute equality' do
        x = Puppet::Application::Fix::Model::Issue.new(mnemonic: 'abc', section: '1.1.1', name: 'test-issue')
        y = Puppet::Application::Fix::Model::Issue.new(mnemonic: 'abc', section: '1.1.1', name: 'test-issue')
        z = Puppet::Application::Fix::Model::Issue.new(mnemonic: 'abd', section: '1.1.1', name: 'test-issue')
